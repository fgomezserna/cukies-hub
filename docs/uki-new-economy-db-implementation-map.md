...
| Claims | Allocations finales de los cierres diario/semanal, Merkle reproducible, API de proofs y worker testnet-only implementados. El worker separa claims, transferencias 80/10 y quema 10, firma de forma durable y espera evidencia del indexador. La autoridad del owner `0xba84...7820` está cargada en Coolify solo como secreto runtime. El primer cierre real contra los contratos UKI activos (`reward-daily:2026-08-21`) confirmó 400.000/50.000/50.000 UKI con tres receipts `success` el 01-09-2026. | Observar el drenaje cronológico del backlog y ejecutar un claim de usuario sobre el distributor activo cuando un cierre contenga beneficiarios reclamables. El canary aislado BSC97 ya publicó y reclamó 10 tokens de prueba el 20-08-2026 (`publish 0x5d44635e...9503`, `claim 0xa4e92d08...e104`). |
...
| Ranking semanal | Solo partidas con créditos del pool; rank #1-#9, subida/bajada semanal y mínimos. | Implementado v3: mejor score raw, desempate temporal, snapshot sellado y runtime independiente. La aplicación busca el último rango histórico anterior, por lo que una semana sin actividad no reinicia al jugador en #5. Gate: `pnpm staging:weekly-rewards:verify-local`. |
| Treasure Hunt | El hub crea `game_economy_sessions`, reserva recursos y liquida; el juego no decide rewards. | Integración v3 completa: autoridad económica, checkpoints, cuotas 30/10, abandono, recuperación, periodo anclado a reserva e integración real DApp/iframe. El gate `pnpm staging:game-rewards:verify-local` cubre las cuatro combinaciones de recursos propios/prestados y replay sin doble consumo. |
| Reparto de recompensas | `G=0..7.5`, 2 UKI semanales, reservas ambassador separadas, pools por generación/rareza y Seiku no distribuido. | Implementado v3 con cierre diario, semanal, siete tramos, censo diario y salida plana por wallet. La recuperación de sources pendientes filtra manifiestos existentes antes del límite del batch para que ninguna partida antigua quede oculta. El gate semanal valida conservación y preparación preview-only; la publicación on-chain permanece separada. |
| Programa de embajadores | Atribucion canonica de un nivel y 5% adicional sobre el pago final. Los sponsors directos bloqueados de preventa migran automaticamente, sin nueva firma ni tramite; las nuevas relaciones se aceptan con sesion EVM y codigo opaco. | Implementado con la politica comun `ambassador-direct-v1`: migracion masiva idempotente desde `presale_participants`, perfiles/codigos opacos, endpoint firmado, panel de referidos y comisiones, integracion en Premios, snapshot por run/corte, relacion inmutable y comision no recursiva. Staging/produccion comparten codigo y cambian solo por entorno/red. El referral XP legacy queda eliminado del runtime. Incluido en `pnpm staging:weekly-rewards:verify-local`. |
| Claim diferido | Las partes de pools del bote semanal se dividen en siete disponibilidades durante la semana siguiente. | Implementado con censo de participantes de cada día receptor, de martes a lunes a las 16:00, funding `reserved_no_mint`; el gate semanal demuestra que no se vuelve a mintear. |
| UKI no distribuido | `dailyCapRaw de la regla - todas las asignaciones/reservas del día`; residuos posteriores del semanal siguen el mismo destino. | Implementado: 500,000 en Stage actual, cap futuro versionable solo desde un corte no abierto, y reparto 80% tesorería, 10% bucket conjunto marketing/desarrollo y 10% reducción de supply, tanto diario como semanal. El gate semanal conserva el importe hasta el último wei. |
