...
| --- | --- | --- | --- |
| 12-J1 · Jugar 1 | Terminar y guardar la partida; resolver el aviso persistente; volver a iniciar 1P sin reconectar. La captura muestra 24 puntos con créditos y Cukie propios. | D12-1 · #418; aviso compartido con D12-3 | PR449 en revisión sobre `cc0f09b7`. La raíz detecta que GAME_READY antes de load se pierde y deja Jugar1P en espera; corrección solicitada con regresiones de ambos órdenes. Pruebas de finish adicionales no equivalen por sí solas a acreditar UKI en Mongo. Sin despliegue. |
...
| 12-C2 · Créditos 2 | Finalizar el corte, mostrar créditos vigentes y permitir configurar el reparto siguiente con una fuente actual. | D12-3 · #421 | PR450 draft, `a2ba5c09`: lectura de varios runs legítimos y pruebas de agregación listas para revisión. El texto de PR requiere normalización; no se ha desplegado ni aceptado el lote completo. |
...
| 12-A1 · Embajadores 1 | El enlace activo `cw-876fe531558b` funciona estable en producción; distinguir indisponibilidad temporal de invitación inválida. | D12-4 · #424 · PR448 | Producción sirve main `cf08e813`; Coolify1554 terminó el 12-Sep a las 13:55 UTC y health confirma SHA. RPC singular403 y otros tres configurados200/chain56: causa comprobada. Diez consultas nuevas consecutivas del enlace con curl devolvieron200 (723–946ms); página200. Corrección verificada en producción. Backport staging `7fd0a92a` en validación. |
| 12-A3 · Embajadores 3 | Mostrar el alias público disponible encima de la wallet; fallback a dirección si no existe. | D12-4 · #424 · PR448 | Código revisado y desplegado en producción `cf08e813`. Lee alias semanal elegido, alias runtime elegido y username público exacto con tiempos acotados. El ejemplo solo tiene alias automático y ningún username público; wallet es el fallback correcto. Propagación staging en preparación. |
...
| 12-O2 · Otros 2 | Tras staking/retirada UKI o NFT confirmados, mostrar saldo, cupos, créditos y avisos coherentes sin recargar; comprobaciones adicionales en segundo plano. | D12-3 · #421 / #419 | Sigue en implementación: la raíz rechazó una prueba que llamaba coherentes a valores Master/Credits diferentes. Se exige expectativa/readback UKI y NFT, respuestas fuera de orden y cambio de wallet, conservando gracia y créditos ganados. No es una validación humana pendiente. |

### Lotes de PR y propietarios

| Lote | Tarea | Rama y destino | Límites y entrega |
| --- | --- | --- | --- |
| D12-1 | **12-Sep · Partidas, ranking y premios**, tarea `01a095b4-c1c5-7591-979b-82bd2025c076` | `codex/doc12-game-settlement` → `staging` | Guardado/settlement, ranking, reserva y lectura de premios; contraste de O1 existente. #418 coordina y #423 recibe la evidencia de cierre. PR449 en corrección de handshake; trabajo y revisión de concurrencia en `17f5`. No aceptada aún. |
...
| D12-3 | **12-Sep · Créditos y actualización de cupos**, tarea `01a095b3-7ec0-7280-8d5f-694bf44a18bc` | `codex/doc12-credit-coherence` → `staging` | Créditos/cortes y coordinación compartida de saldo, cupos y avisos. Dueño del refresco global; D12-1/2 consumen su contrato. **En curso**, worktree `54f6`. PR450 draft, `a2ba5c09`; C2 en revisión y O2 requiere completar coordinación/fixtures tras rechazo de evidencia insuficiente. |
| D12-4 | **12-Sep · Hotfix de embajadores**, tarea `01a095b2-4772-7df0-afb8-db9254e56135` | `codex/doc12-ambassadors-hotfix` desde `main`, después propagación a `staging` | PR448 integrada en main `cf08e813`. Revisión raíz y 173 suites/1510 tests del candidato `7509735d` correctos. Coolify1554 finalizada y health confirma main `cf08e813`. Diez respuestas200 de la invitación exacta y página200 verificadas. Propagación staging en curso. Worktree `2b5e`. |
