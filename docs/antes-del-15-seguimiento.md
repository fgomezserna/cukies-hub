...
| D | **MIGRACION POR COMPLETAR**: migrar toda funcionalidad legacy al Hub con infraestructura nueva, auth, datos y contratos seguros; dejar de usar el repo antiguo. | Decision de producto e [inventario de migracion](legacy-marketplace/README.md); 2026-09-07: 14 contratos, 34 contenedores y 16 instancias de base. 2026-09-09: Game/matchmaking portados y probados localmente en Hub; [evidencia del stack](legacy-marketplace/evidence/2026-09-09-world-stack-port.json). | Importacion preview, identidad verificada y paridad por flujos completos; coordinar [#160](https://github.com/fgomezserna/cukies-hub/issues/160). Perfil World desactivado y sin despliegue Stage/Prod; cero consumidores runtime antes de retirar. |
...
| Game y matchmaking | Port local en `packages/world-api`, `world-matchmaking` y `world-shared`; 135 + 9 declaraciones HTTP conservadas. Build, tests e imágenes Docker con smoke de caída/recuperación Redis verificados el 2026-09-09; [evidencia](legacy-marketplace/evidence/2026-09-09-world-stack-port.json) | Sin despliegue Stage/Prod de este port. Los servicios legacy siguen inventariados, sin revalidación live en esta pasada. Perfil `world-runtime` opt-in en el mismo Compose del Hub | Reconciliar datos/IDs, conectar emisor de sesiones Hub a identidad verificada, validar cliente Unreal y pasar a un único escritor antes de retirar upstreams; [operación y límites](../infrastructure/world/README.md) |
