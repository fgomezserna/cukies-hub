...
- Filas **2A-C y 5**: el lote local del CTA `Vender` en PR359 tiene un P2 de
  revisión pendiente (2026-09-09): debe separar la colección Legacy BSC56 del
  origen del anuncio y permitir republicación UKI con configuración exacta.
...
- Lote de elegibilidad `MC09 / SV01–SV04 / V12`, 2026-09-09, sobre base
  ...
  código `1f13cd99`: **revisión independiente no aprobada; P2 en cola**.
  El parche exige propiedad/disponibilidad y conserva la identidad solicitada,
  pero prioriza siempre Legacy para su colección BSC56, incluso si UKI admite
  expresamente esa colección o ya existe una orden UKI sobre el NFT.
  Eso contradice la convivencia y republicación de las reglas vigentes
  (`docs/uki-current-operating-rules.md`, bloque Marketplace Legacy/UKI).
  Plan acotado para el próximo hueco de ejecución:
  1. Separar origen del anuncio activo de destinos elegibles de publicación;
     detalle/cancelación conservan su origen y contrato exactos.
  2. Mantener acceso a UKI para colección Legacy BSC56 cuando readiness,
     cadena y allowlist exactas lo permitan, sin convertir anuncios existentes;
     para republicar se exige cancelar previamente el anuncio Legacy.
  3. Conservar rechazo de colección Legacy56 atribuida a97 y todas las guardas
     de propiedad, custodia y estado. Añadir regresiones de doble elegibilidad
     BSC56, orden UKI activa sobre esa colección y anuncio Legacy sin migración.
  Corrección en cola tras Datos/juego; no se inicia otro agente, activación V2
  completa ni infraestructura. Merge/deploy sujetos a nueva revisión y handoff INFRA.
  ...
  Evidencia focal: 27 tests y typecheck OK, con BSC56/BSC97, colección Legacy en
  red contradictoria, propiedad/custodia/estado, token homónimo y URL inválida.
  Gates del código `1f13cd99` (no cubren el P2 descubierto): `pnpm dapp lint`
  sin avisos y `pnpm dapp test --runInBand`
  con 234 suites / 1.895 tests OK; `git diff --check` OK.
