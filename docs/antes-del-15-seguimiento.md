...
| 12-C2 · Créditos 2 | Finalizar el corte, mostrar créditos vigentes y permitir configurar el reparto siguiente con una fuente actual. | D12-3 · #421 | PR450 se entrega como lote acotado de lectura de créditos/avisos. Prueba O2 insuficiente retirada en `7a2ac426`. Raíz acepta el candidato junto a449:275 suites/2407 tests,100 pruebas focales, tipos y lint correctos. Desplegada en staging `af41cce6`; CI34700104183 y SHA servido verificados a las14:56UTC. Comprobación remota14:50UTC detecta otro fallo ejecutable: scheduler de créditos HTTP503/DOMAIN_CONFLICT desde09:47UTC,593 fallos y ningún run del periodo actual. RCA: `nft_lock_asset_identity_invalid` rechaza el identificador canónico completo de #98000002; además hay tres Transfer de TOKEN_V2 en dead letters. PR452 `07255a28` corrige la identidad canónica compuesta y evita bloquear UKI por un incidente propio del reconciliador NFT; raíz acepta el diff;275 suites/2410 tests, tipos y lint correctos. Integración y despliegue en preparación. La incidencia NFT falsa ya abierta requiere una resolución acotada tras comprobar identidad y estado actuales; tarea D12-5 prepara el dry-run. C2 sigue sin evidencia de nuevo corte operativo. O2 permanece separado. |
...
| 12-O2 · Otros 2 | Tras staking/retirada UKI o NFT confirmados, mostrar saldo, cupos, créditos y avisos coherentes sin recargar; comprobaciones adicionales en segundo plano. | D12-3 · #421 / #419 | Implementación separada de PR450. La raíz exige reutilizar expectativas UKI y prueba NFT por identidad/operación, consultar Master+Credits aunque una pantalla no esté montada, y evitar declarar coherentes dos respuestas antiguas. Candidato O2 `8d291425` revisado el12-Sep: dos hallazgos abiertos. La hidratación puede completar el pending con Master actualizado pero Credits antiguo; las expectativas de operaciones sustituidas pueden coexistir y bloquear indefinidamente la convergencia. Corrección y regresiones conjuntas asignadas a la misma tarea; todavía sin PR/deploy. Conserva gracia/créditos; no depende de validación humana. |
...
| D12-3 | **12-Sep · Créditos y actualización de cupos**, tarea `01a095b3-7ec0-7280-8d5f-694bf44a18bc` | `codex/doc12-credit-coherence` → `staging` | Créditos/cortes y coordinación compartida de saldo, cupos y avisos. Dueño del refresco global; D12-1/2 consumen su contrato. **En curso**, worktree `54f6`. PR450 C2/avisos desplegada en staging `af41cce6`, CI34700104183 y health verificados a las14:56UTC; O2 se entrega después en PR separada dentro de la misma tarea. Rama O2 actual `codex/o2-readback-coherence`, candidato `8d291425`: revisión Astra devuelve dos fallos de hidratación/sustitución; corrección focal en curso. |
| D12-5 | **12-Sep · Desbloquear cortes por identidad…**, tarea `01a09621-6fb0-75c1-bce1-2d3caa5d8296` | `codex/doc12-nft-source-integrity` → `staging` | Ampliación C2 tras evidencia remota: contrato de identidad canónica NFT, alcance de incidentes y eventos pendientes. Worktree `acfe`. PR452 `07255a28` aceptada por revisión raíz; candidato integrado `b14dff86`:275 suites/2410 tests, tipos y lint correctos. Pendiente de merge/despliegue;139 pruebas focales previas correctas en la tarea. Dry-run operativo en curso para resolver exclusivamente el falso incidente demostrado; no elimina dead letters ni reconstruye partidas/premios históricos. Operaciones remotas quedan en raíz, con prueba y selectores exactos. |
