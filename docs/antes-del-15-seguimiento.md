...
| D-stage | **RÉPLICA LEGACY EN STAGING · PR454 EN CORRECCIÓN**: script y plan local preparados; la revisión Astra impide aplicar el candidato por fallos de idempotencia, integridad del bundle, concurrencia, aislamiento97/56, sanitización y tipos BSON. **Sin exportación ni carga remota**. Encargo exclusivo separado de la consolidación general. | 12-Sep: tarea **Réplica legacy de main en staging**, `01a096a5-848d-7292-af74-1e69300dc555`, Luna Max, worktree `a3e8`, rama `codex/stage-legacy-replica`; [PR454](https://github.com/fgomezserna/cukies-hub/pull/454) draft, candidato `c41995ea`. CI34709434732 validate-delivery SUCCESS; 9 pruebas del script y107 de guarda reportadas. La revisión aislada reproduce duplicación y acepta bundles alterados; se remitió una corrección acotada, incluida compatibilidad de TRON/Points/relaciones y reconciliación de identidades. [Evidencia y hallazgos](legacy-marketplace/evidence/2026-09-12-production-source-audit.json). | Revisar nuevo SHA, exportación sanitizada real y dry-run por colección/corte antes de cargar/proyectar selectivamente en `cukies-legacy-staging` y `cukieshub-new-staging`. Preservar datos97/créditos/premios/auth y contratos; main solo lectura. Acreditar paridad, duplicados/huérfanos, repetición sin mutaciones y QA de lecturas antes del cierre. Raíz y auditora reciben hitos; seguimiento separado `coordinar-r-plica-legacy-staging`. No cambia escritores/tráfico/permisos ni reabre los diez puntos12-Sep. |
