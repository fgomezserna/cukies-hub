...
| D-stage | **BASE ÚNICA DE STAGING · DATOS CORE COPIADOS, CONEXIONES EN CURSO**. Destino `cukieshub-new-staging`; el usuario autoriza copiar legacy tal cual y reemplazar pruebas. | 12-Sep19:00UTC: 40 colecciones legacy y la vista `wallets_groups` copiadas; 17.464 NFT, 25.662 movimientos NFT, 23.529 movimientos de puntos. También cuatro colecciones Hub de staging (339 usuarios/5.806 sesiones/340 wallets). [Evidencia](legacy-marketplace/evidence/2026-09-12-production-source-audit.json). Tarea **Unificar las bases de datos en staging**, `01a096a5-848d-7292-af74-1e69300dc555`; [PR454](https://github.com/fgomezserna/cukies-hub/pull/454) se adapta al alcance mínimo. | Completar Game/Learn/Ludo y cambiar todas las conexiones de staging después de revisar el código compatible; desplegar y verificar cada consumidor. **La copia no equivale todavía a unificación operativa**. Producción sigue funcionando/escribiendo normalmente: esta operación solo lee sus datos para copiarlos. No hay cambio de permisos, conexiones ni despliegue de producción. Coordinación ACTIVE; eliminar al completar. Ciclos1800s; 30→15min cancelado. Los diez puntos12-Sep mantienen su cierre técnico. |
