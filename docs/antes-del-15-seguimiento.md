...
| D | **MIGRACION LEGACY Y WORLD POR COMPLETAR; PORT Y PUBLICACION CI VERIFICADOS**: faltan datos, identidad y consumidores en la infraestructura nueva. Learn/Ludo, auth/GraphQL y operación Unreal/Agones siguen en el alcance total. | 2026-09-10 04:19:57 UTC: [PR346](https://github.com/fgomezserna/cukies-hub/pull/346) cerrada como sustituida por372/373/374; origen `f4cae5e307fed2a84acf0de3e33d0dae9b25b0a1`, rama y worktree originales intactos. De205 deltas se importaron200 archivos:199 conservan blob/modo exacto y el README se adaptó al CI. Procedencia histórica y checkout principal `6f13b05` preservados. Stage código `26ae560`, CI34435609267SUCCESS: World API/MM publicados y registrados, seis referencias activas y15 contenedores protegidos sin cambios. [Evidencia y cobertura](evidence/2026-09-10-world-ci-integration.json). | CI World cerrado; nuevo runtime/escrituras OFF. Se inventarían ocho contenedores Legacy activos preexistentes en `service-vokgw84g4g4cs0woog0g8o0k`: auth, GraphQL, REST, Learn, Ludo, game, marketplace y matchmaking. El baseline no los incluía: no se afirma comparación histórica ni obsolescencia. Reconciliar consumidores, datos/IDs, emisor Hub→World y cliente; acreditar un único escritor antes del cambio de tráfico y cero consumidores antes de retirar servicios. Fuente World `01a085a9-ce55-75e0-831e-b356608c2ab9` conservada; Legacy blockchain `01a08727-0cdb-7993-bbab-6829bcf3d333` atiende el lote independiente de identidad de padres.  Auditoría de producción recibida12-Sep de la tarea **Auditoría fuerte de sincronización BD…**: ambas colecciones reportan17464NFT, pero no se acredita paridad. La cifra2939 se calculó por network+cukiNumber, con5311duplicados; se conserva como señal de snapshot, no como2939NFT incorrectos. Falta reconciliar red/colección/token, owner/custodia y corte común; el contraste tokenId/_id solo prueba cobertura. Mantener un escritor por dominio y migrar lectores antes de poner Legacy en solo lectura; no convertir filas heredadas en logs verificados sin procedencia. [Informe recibido y límites](legacy-marketplace/evidence/2026-09-12-production-source-audit.json). Alcance D separado del documento de pruebas12-Sep, sin activar cambios en producción. |
| D-stage | **RESUELTO · STAGING UNIFICADO Y OPERATIVO**: Hub/auth, legacy, indexer/economía, cards, bridge y schedulers resuelven una única base lógica `cukieshub-new-staging` en el Mongo dedicado de `192.168.1.221:27018`; las colecciones legacy mantienen sus nombres/namespaces dentro de esa base. `eventlog` dejó de ser una dependencia runtime y los guards rechazan que reaparezca. | Merge `4f51e4d3403869942d67ca57629114c3bc963a2b` (PR458), workflow de imágenes con validaciones verdes; verificación live 2026-09-13: `/api/health` y `/api/ready` 200, APIs legacy levantadas, `login`/`marketplace` recreados sin `NX_TRON_DB`, producción sin cambios. [Runbook de Stage](../infrastructure/ci/staging-data-handoff.md) y [evidencia de unificación](../docs/legacy-marketplace/evidence/2026-09-07-runtime.json). | Mantener `DATABASE_URL` como fuente canónica y derivar todos los aliases; no ejecutar el refresh histórico de dos bases. Antes de cualquier carga o limpieza, snapshot y reconciliación con rollback. Producción continúa solo en preparación. |
...
| --- | --- | --- | --- |
| 12-J1 · Jugar 1 | Terminar y guardar la partida; resolver el aviso persistente; volver a iniciar 1P sin reconectar. La captura muestra 24 puntos con créditos y Cukie propios. | D12-1 · #418; aviso compartido con D12-3 | PR449 código `56462244` aceptado: handshake petición/respuesta corrige ready→load, load→ready y recarga del iframe; reintentos acotados distinguen carreras transitorias de errores definitivos. Base staging actualizada sin conflictos; raíz:274 suites/2395 tests, tipos y lint correctos. Integrada en staging `fb841f84` el12-Sep14:26UTC; CI34699288472 completada; web y juego sirven el SHA exacto `fb841f84`, verificado el12-Sep14:42UTC. |
| 12-J3 · Jugar 3 | Registrar la puntuación, reservar el bote y permitir el reparto del periodo correcto. | D12-1 · #418 / #423 | PR449 desplegada en staging `fb841f84`; web y juego verificados el12-Sep14:42UTC. El cierre vuelve a leer estado y reintenta fences/colisiones transitorias conservando ranking y reserva idempotentes. Consulta remota12-Sep15:07UTC:0 sesiones nuevas desde el despliegue14:41:05UTC. C2 vuelve a estar operativa desde16:06UTC; queda probar una partida nueva autenticada; sin reconstrucción histórica. |
...
| 12-C2 · Créditos 2 | Finalizar el corte, mostrar créditos vigentes y permitir configurar el reparto siguiente con una fuente actual. | D12-3 · #421 | **Corregido y operativo en staging.** PR450 corrige lectura/avisos y PR452 reconoce identidad NFT compuesta; despliegue `00e95ecd`/CI34703683490 verificado el12-Sep16:05UTC. Raíz resolvió solo el falso incidente con selector exacto y prueba ownerOf en bloque seguro130627853. A las16:09UTC, ambos cortes16:00–16:30 están abiertos:300 créditos NFT+200 UKI aplicados; watermarks actualizados y scheduler sin fallos. Se conservaron lock invalidado y tres Transfer históricos, sin replay. A las17:09UTC se comprueban otros300 créditos NFT+200 UKI aplicados en el corte17:00–17:30 y scheduler sin fallos; los cortes16:30/17:00 se abren automáticamente. O2 sigue separado en PR453. |
| 12-W1 · Premios 1 | Acreditar los UKI de una partida nueva y mostrar su estado e importe canónicos. | D12-1 · #418 / #423 | PR449 desplegada en staging `fb841f84`; web y juego verificados el12-Sep14:42UTC: los reintentos completan la misma liquidación y mantienen autoridad/payload originales. 67 pruebas existentes de economía y contabilidad correctas según la tarea; no se afirma una acreditación live todavía. |
| 12-W3 · Premios 3 | Reservar los UKI semanales de las partidas y llevarlos al cierre y reparto correspondientes. | D12-1 · #418 / #423 | PR449 desplegada en staging `fb841f84`; web y juego verificados el12-Sep14:42UTC: mantiene el registro idempotente de la fuente semanal y los guards del periodo; no cambia reglas ni salta validaciones de cierre. |
| 12-A1 · Embajadores 1 | El enlace activo `cw-876fe531558b` funciona estable en producción; distinguir indisponibilidad temporal de invitación inválida. | D12-4 · #424 · PR448 | PR448 producción `cf08e813` verificada: diez consultas nuevas200 y página200. Backport PR451 integrado a staging `2c7a3882` a las 14:12UTC; CI34698620345 terminó correctamente; health sirve staging `2c7a3882`. |
| 12-A3 · Embajadores 3 | Mostrar el alias público disponible encima de la wallet; fallback a dirección si no existe. | D12-4 · #424 · PR448 | PR448 desplegada en producción. Alias elegido semanal/runtime y username público exacto; wallet si no existen. PR451 desplegada en staging `2c7a3882`, SHA verificado. Invitación propia de staging `cw-56918691f339`:200 con nombre público; otra invitación no elegible devuelve404. Lecturas verificadas sin mutar perfiles. |
...
| 12-O2 · Otros 2 | Tras staking/retirada UKI o NFT confirmados, mostrar saldo, cupos, créditos y avisos coherentes sin recargar; comprobaciones adicionales en segundo plano. | D12-3 · #421 / #419 | **Entrega técnica completada y desplegada.** PR453 integrada en staging `1fcac2b9`; CI34707208089 completada y SHA servido exacto verificado12-Sep17:16UTC. `15067005` acredita el par Master+Credits por operación y reanuda tras reset; la regresión97→56→97 termina sin bloqueo ni nuevas lecturas. Raíz:275 suites/2418 tests, tipos y lint correctos. Gracia/créditos ganados conservados; la firma con wallet personal queda como prueba humana, no como tarea técnica bloqueada. |

### Lotes de PR y propietarios

| Lote | Tarea | Rama y destino | Límites y entrega |
| --- | --- | --- | --- |
| D12-1 | **12-Sep · Partidas, ranking y premios**, tarea `01a095b4-c1c5-7591-979b-82bd2025c076` | `codex/doc12-game-settlement` → `staging` | Guardado/settlement, ranking, reserva y lectura de premios; contraste de O1 existente. #418 coordina y #423 recibe la evidencia de cierre. PR449 código `56462244` aceptado tras revisión de handshake y retries. Base staging incorporada;274 suites/2395 tests, tipos y lint correctos. Integrada en staging `fb841f84`; CI34699288472 completada; web y juego sirven `fb841f84` y están saludables. |
...
| D12-3 | **12-Sep · Créditos y actualización de cupos**, tarea `01a095b3-7ec0-7280-8d5f-694bf44a18bc` | `codex/doc12-credit-coherence` → `staging` | **Entrega final completada.** PR450 y PR452: C2 operativo, con nuevos repartos16:00/16:30/17:00 y500 créditos aplicados en la comprobación17:09UTC. PR453: O2 revisado y desplegado en `1fcac2b9`, CI34707208089 y health exacto verificados17:16UTC.275 suites/2418 tests, tipos y lint correctos. |
| D12-5 | **12-Sep · Desbloquear cortes por identidad…**, tarea `01a09621-6fb0-75c1-bce1-2d3caa5d8296` | `codex/doc12-nft-source-integrity` → `staging` | PR452 integrada y desplegada en staging `00e95ecd`; CI34703683490 y SHA servido comprobados. Raíz:275 suites/2410 tests, tipos y lint correctos. Resolución del único falso incidente a las16:05UTC mediante CAS y proofRPC; lock invalidado y dead letters conservados. A las16:09UTC,500 créditos nuevos aplicados en los dos cortes actuales y scheduler sin fallos. Entrega técnica y verificación operativa de C2 completadas. |
| D12-4 | **12-Sep · Hotfix de embajadores**, tarea `01a095b2-4772-7df0-afb8-db9254e56135` | `codex/doc12-ambassadors-hotfix` desde `main`, después propagación a `staging` | PR448 integrada en main `cf08e813`. Revisión raíz y 173 suites/1510 tests del candidato `7509735d` correctos. Coolify1554 finalizada y health confirma main `cf08e813`. Diez respuestas200 de la invitación exacta y página200 verificadas. PR451 integrada a staging `2c7a3882`; CI34698620345 terminó correctamente; health sirve staging `2c7a3882`. Worktree `2b5e`. |

Cada tarea usa Luna Max y entrega PR con reproducción, cambios y pruebas
...
Entrega técnica de esta revisión completada el12-Sep17:16UTC. La automatización
`coordinar-pruebas-cukies-12-sep` se eliminó tras verificar el despliegue final
PR453. Las pruebas humanas de nuevas partidas, premios y operaciones autenticadas
permanecen indicadas por punto; no mantienen encargos técnicos abiertos.
...
| A1 | Invitación válida intermitente | Corregido y desplegado12-Sep, main448/staging451 | Causa RPC y consultas activas verificadas; véase12-A1. |
| A2 | Aviso tras confirmar embajador | Desplegado main426/staging428 | Comprobación positiva autenticada. |
| A3 | Nombre y wallet del embajador | Actualizado12-Sep, main448/staging451 | Alias público servido en invitación de staging; fallback sin alias en producción. Véase12-A3. |
...
- [ ] Validar `pnpm guard:staging:test` y `pnpm guard:staging` con UUID de Coolify
  `u4s804o4wwcckowgk0woo4wg`, rama `staging`, chain `97` y la única base lógica
...
| Producers/consumer/sync y eventos | Chain-indexer/importers portados parcialmente; retirada reproducible de `getter-bsc`, `getter-tron` y `setter` preparada en `cukiesworld-stack` PR #19 (`d52916fc`) | 15/09/2026: los tres contenedores y sus declaraciones se retiraron de CT2051/CT2050; `legacy-chain-indexer` de Stage sigue `Exited (1)` y la paridad aún no está cerrada | `legacy-chain-indexer` activo como único writer BSC/TRON, cursores avanzando y paridad on-chain/Mongo/API; no reactivar getters/setter ni usar `processedEvents` como fallback |
