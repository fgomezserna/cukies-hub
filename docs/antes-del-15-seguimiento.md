...
| --- | --- | --- | --- |
| INFRA | **MONGO STAGING MIGRADO AL LXC; CI DE IMAGENES EN VALIDACION**: LXC2007, instancia dedicada `27018`, Mongo 7, fuera del Compose; builder VM1012 instalado. Coolify conserva la aplicacion. | 2026-09-09, Stage `53088a3`; [reconciliacion](../infrastructure/ci/2026-09-09-mongo-lxc-evidence.json): 577.434 documentos, hashes, indices y vistas sin diferencias; Prisma y avance del indexador verificados. | Integrar y activar [CI con Nx/BuildKit](deployment-environments.md). Autodeploy Git pausado y gate CI en false durante la transicion; Mongo original y backups conservados. Produccion no modificada. |
...
| 2A-C | **MARKETPLACE LEGACY OPERATIVO EN STAGING; PARIDAD HISTORICA PENDIENTE**: catálogo publico verificado contra contratos BSC/TRON, ficha rica y permalink histórico, venta/compra/cancelación/cambio de precio con revalidación final, bloqueo de doble envío y reconciliación Mongo tras recibo. El catálogo descarta candidatos obsoletos sin mutar el conjunto mientras pagina; el inventario vendedor pagina más de 60 NFTs. UKI V2 permanece cerrado de forma segura. | [PR #342](https://github.com/fgomezserna/cukies-hub/pull/342), Stage `32b27fc`, deploy `v48k4ggsgokkg80cw4o80s4w` terminado 2026-09-09 10:09:56 UTC; BSC #1000000000029 = 0,195 BNB y TRON #4000000007954 = 900 TRX contrastados live. [PR #344](https://github.com/fgomezserna/cukies-hub/pull/344) añade continuidad, CAS y guardas; los gates se repiten sobre la base final antes del rollout. | El worker `legacy-chain-indexer` sigue inactivo y su preview apunta a un destino distinto; no se afirma paridad general ni orden global de precio ante cambios externos todavía no proyectados. No se activa sin provisionar destino/cursor y resolver RPC de archivo BSC. Completar backfill/replay y pruebas transaccionales controladas. Producción/main no se modifica. |
...
| Actualizacion y acciones | **Recurrencia corregida localmente; despliegue bloqueado, 2026-09-09.** A las 11:23 UTC, Stage `bae6f3b` ofrece staking Master para `98000001`, `98000003` y `98000004` aunque siguen en el Pool anterior. El candidato `50d27f5`, sobre Marketplace `53088a3`, comparte custodia y ciclo transaccional NFT; conserva inventario y pendientes por identidad. Contraste RO del candidato a las 11:40 UTC y 227 suites / 1.841 tests correctos. PR [#347](https://github.com/fgomezserna/cukies-hub/pull/347), rama `codex/nft-operation-recurrence`. Tras agotar `/srv`, el operador informa recuperacion a 19.150.913.536 bytes libres y servicios sanos a las 12:07 UTC; mantiene la ventana para otro arreglo Marketplace. El siguiente deploy exige traspaso de ventana y margen para el consumo del build por encima del suelo de 10 GiB. | Pendiente merge/deploy y QA de la version servida cuando el operador libere la ventana y se verifique capacidad suficiente. Las simulaciones y tests no equivalen a completar una transaccion de wallet ni a despliegue. |
...
| UX-09 | Catalogo conjunto Legacy/UKI, identidad por cadena/coleccion y navegacion paginada | **Legacy verificado live en Stage `32b27fc`**: 24 tarjetas con precio autoritativo BSC/TRON, ficha rica y permalink #8733, venta/compra/cancelación/cambio de precio, fuente parcial explícita e inventario vendedor 60+. PR #344 añade reconciliación post-recibo con CAS; el catálogo muestra precio live pero conserva el orden/cursor indexado hasta que exista proyección continua. UKI V2 continúa `unavailable` porque falta contrato/configuración BSC97; no se representa como cero |
