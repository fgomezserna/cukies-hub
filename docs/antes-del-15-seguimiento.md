...
| 2A-C | **EN IMPLEMENTACION DEL BLOQUE DE EVENTOS**: cobertura ABI local contrastada para 14 contratos legacy y 10 perfiles nuevos (75 y 87 relaciones contrato/evento). Replay de breeding y correlacion atomica del bridge verificados localmente, incluida concurrencia Mongo. Legacy y UKI conviviran en una lista con filtros y distintivo Legacy por tarjeta. | Decision de producto y pruebas locales; 2026-09-07; catalogo `packages/chain-indexer/src/config/event-manifest.ts` y pruebas `test/event-coverage.test.ts`, `test/bridge-cas.integration.test.ts`; [reglas funcionales](uki-current-operating-rules.md#contratos-legacy-eventos-y-convivencia-decision-del-2026-09-07). | Cerrar verificacion integrada del worker aislado y sus fuentes live; despues activar Stage, reconciliar datos e integrar lista/filtros/acciones. La cobertura ABI no acredita despliegue. Stage observado `d4bc372`: API UKI 503; main `fb2b190`: API UKI 404, sin inferir ausencia de contratos. |
