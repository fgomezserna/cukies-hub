...
| 12-M2 · Cukie Master 2 | Tras aprobar el NFT y cancelar el depósito, ofrecer Continuar depósito; conservar la elegibilidad y revalidar antes de firmar. | D12-2 · #419 | D12-2 ha acotado la discrepancia entre aprobación persistida y `canDeposit` del inventario. Corrección en curso: continuar solo con identidad y custodia elegibles, revalidando propiedad y permisos en cadena. Sin despliegue nuevo. |
...
| 12-A3 · Embajadores 3 | Mostrar el alias público disponible encima de la wallet; fallback a dirección si no existe. | D12-4 · #424 | La API devuelve `ambassadorPublicName:null`. Revisión raíz: priorizar el alias elegido del perfil semanal que ya usa staging (#429), después el elegido de la campaña runtime y el username público exacto. Ignorar alias automáticos y conservar el fallback si otra fuente tarda. Implementación en corrección; sin escrituras de perfil ni asociaciones desde rankings anónimos. |
