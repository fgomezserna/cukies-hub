...
| 2A-C | **MARKETPLACE LEGACY OPERATIVO EN STAGING; PARIDAD HISTORICA PENDIENTE**: catálogo publico verificado contra contratos BSC/TRON, ficha rica y permalink histórico, venta/compra/cancelación/cambio de precio con revalidación final, bloqueo de doble envío y reconciliación Mongo tras recibo. El catálogo actualiza candidatos obsoletos y el inventario vendedor pagina más de 60 NFTs. UKI V2 permanece cerrado de forma segura. | [PR #342](https://github.com/fgomezserna/cukies-hub/pull/342), Stage `32b27fc`, deploy `v48k4ggsgokkg80cw4o80s4w` terminado 2026-09-09 10:09:56 UTC; BSC #1000000000029 = 0,195 BNB y TRON #4000000007954 = 900 TRX contrastados live. [PR #344](https://github.com/fgomezserna/cukies-hub/pull/344), código `7697cd1`, añade continuidad y guardas; lint, tipos, build y 225 suites / 1.821 tests correctos sobre base `bae6f3b` antes del rollout final. | El worker `legacy-chain-indexer` sigue inactivo y su preview apunta a un destino distinto; no se afirma paridad general ni se activa sin provisionar destino/cursor y resolver RPC de archivo BSC. Completar backfill/replay y pruebas transaccionales controladas. Producción/main no se modifica. |
...
- Cierre Marketplace del 2026-09-09 en `staging`: el catálogo no publica todo
  ...
  degrada cada red por separado y reconcilia de forma idempotente los candidatos.
  Las acciones vuelven a comprobar propietario, anuncio, precio, cuenta y red
  inmediatamente antes de firmar, mantienen el bloqueo hasta el recibo y actualizan
  la misma base `cukies-legacy-staging` después de confirmarlo.
