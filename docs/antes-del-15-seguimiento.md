...
| --- | --- | --- | --- |
| INFRA | **WEB Y WORKERS SEPARADOS; CIERRE GRADUAL EN VALIDACIÓN**: web app32, workers app28, Mongo LXC2007 `27018`, builder VM1012 y registry VM1007. | 2026-09-09 21:18 UTC: stage sirve `bdab82c`, [CI 34405168825](https://github.com/fgomezserna/cukies-hub/actions/runs/34405168825) SUCCESS e indexador con digest `0ce94ee1…` y SOURCE_COMMIT verificados; diez workers reconciliados y journal limpio. [Evidencia](../infrastructure/ci/2026-09-09-rolling-delivery-evidence.json): primer relevo 1117/1117 HTTP200; candidato unhealthy 173/173; tres sustituciones posteriores registran un timeout cada una y el ensayo NFT confirma 502/timeout en origen al SIGTERM. Build DApp selectivo `c7b33c1` conservó diez workers; build indexador selectivo sí recreó todo el Compose. Se conserva la evidencia de [Mongo](../infrastructure/ci/2026-09-09-mongo-lxc-evidence.json) y [espacio](../infrastructure/ci/2026-09-09-storage-recovery-evidence.json). | Responsable exclusivo: tarea `01a0859a-cf4d-77a0-a137-6db60a0a0f96`; ventana coordinada con recuperación selectiva NFT antes del siguiente ensayo. Falta validar cierre gradual real y no-op documental; no declarar cero interrupciones. `CUKIES_DELIVERY_MODE=rolling`, gate CI true y autodeploy Git false. [Procedimiento](deployment-rolling-transition.md). Producción conserva `4475baa` en app12; app33 sin tráfico y [PR361](https://github.com/fgomezserna/cukies-hub/pull/361) draft sin integrar. App31 mantiene build independiente. GC y autoscalado no implementados. |
...
| 1 | **EN PROGRAMA DE MIGRACION LEGACY**: bridge Tron -> BSC, con seguridad, E2E y fees dentro del alcance. | Decision de producto; 2026-09-07. | Responsable Legacy `01a08727-0cdb-7993-bbab-6829bcf3d333`; preparar soporte mainnet Legacy desde Stage sin activar relayer ni signers antes de revisión. Revalidar fuente, relayer, rutas, fee y pausa; sin presentar el bridge como cerrado. |
...
| 3 | **EN STAGING, EN PRUEBAS · NUEVO BLOQUEO NFT IDENTIFICADO**: Cukie Master, creditos, pools, prestamos y rewards. | 2026-09-08 19:28 UTC, Stage `67303f6` (PR #329–#333): historial 5→4→0 NFT / UKI 0, ambas rutas al dia y 0 grants QA posteriores a retiradas. Dos ticks waiting renuevan ambas fuentes; Master/Créditos/Resumen coherentes y sin sus avisos incorrectos. 1.746 tests y gates PASS. [Evidencia](legacy-marketplace/evidence/2026-09-08-stage-data-recovery.json). Nueva incidencia 2026-09-09 20:51 UTC, worker `1646602`: un `TOKEN_V2:Transfer` falló a las 16:53 por `price` duplicado en `$set`/`$setOnInsert`, agotó cinco intentos y bloquea créditos NFT; UKI continúa. [Evidencia por ruta y período](evidence/2026-09-09-master-credit-coherence.json). Los 5.400 históricos de la wallet QA corresponden a 3.000 de grants y 2.400 de compensación, sin claves duplicadas y expirados a las 17:00 del 7/9; el DOCX no identifica wallet/período y no permite atribución definitiva. | Responsable `01a0814e-0ea0-7070-877c-701f264243e7` (datos/juego), con implementación Luna high y revisión del coordinador. Parche mínimo y regresión validados en `codex/master-credit-coherence-20260910`: tipos/build PASS, 86 tests PASS y 1 integración Mongo opt-in omitida. Integrar/desplegar y recuperar únicamente el evento fallido con las precondiciones `ownerOf`/cursor del artefacto; comprobar avance NFT y créditos sin manipular saldos ni watermarks. Cierre de [#326](https://github.com/fgomezserna/cukies-hub/issues/326) limitado a la incidencia resuelta el 8/9. Continuan las pruebas amplias de economia/UX en [#289](https://github.com/fgomezserna/cukies-hub/issues/289); Vesting sigue siendo un aviso independiente. Produccion `fb2b190` sin cambios. |
...
| post1-2 | **ANTES DEL 15 · MIGRACION LEGACY**: Cukie Points y crias siguen ligados al bloque de migracion; no son activos POST-15. | Decision de producto; 2026-09-07. | Responsable Legacy `01a08727-0cdb-7993-bbab-6829bcf3d333`: Crías/Points y aclaración contractual Bread, sin confundirlos ni declarar paridad por HTTP 200. Inventario y migracion validada; no ejecutar pausa/corte en este seguimiento. |
