...
test('registers staking, vesting and rewards only when their BSC addresses are explicit', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      'UKI_STAKING:OwnershipTransferStarted',
      'UKI_STAKING:OwnershipTransferred',
      'UKI_STAKING:Paused',
      'UKI_STAKING:Unpaused',
      'VESTING_VAULT:VestingCreated',
      'VESTING_VAULT:TokensReleased',
      'VESTING_VAULT:PresaleVestingConfigFrozen',
      'VESTING_VAULT:PresaleVestingConfigUpdated',
      'VESTING_VAULT:RoleAdminChanged',
      'VESTING_VAULT:RoleGranted',
      'VESTING_VAULT:RoleRevoked',
      'VESTING_VAULT:UnallocatedWithdrawn',
      'REWARDS_DISTRIBUTOR:BatchPublished',
      'REWARDS_DISTRIBUTOR:RewardClaimed',
      'REWARDS_DISTRIBUTOR:BatchClosed',
      'REWARDS_DISTRIBUTOR:ExcessRecovered',
      'REWARDS_DISTRIBUTOR:OwnershipTransferStarted',
      'REWARDS_DISTRIBUTOR:OwnershipTransferred',
      'REWARDS_DISTRIBUTOR:Paused',
      'REWARDS_DISTRIBUTOR:Unpaused',
    ],
  );
  assert.throws(
...
test('registers the explicit verified BSC NFT sources without mainnet address fallback', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      `TOKEN:ApprovalForAll:${TOKEN}`,
      `TOKEN:MinterAdded:${TOKEN}`,
      `TOKEN:MinterRemoved:${TOKEN}`,
      `TOKEN:OwnershipRenounced:${TOKEN}`,
      `TOKEN:OwnershipTransferred:${TOKEN}`,
      ...
      `MARKETPLACE:OwnershipRenounced:${MARKETPLACE}`,
      `MARKETPLACE:OwnershipTransferred:${MARKETPLACE}`,
      `MARKETPLACE:Paused:${MARKETPLACE}`,
      `MARKETPLACE:Unpaused:${MARKETPLACE}`,
      `BRIDGE:JumpInBridge:${BRIDGE}`,
      `BRIDGE:JumpOutBridge:${BRIDGE}`,
      `BRIDGE:OwnershipRenounced:${BRIDGE}`,
      `BRIDGE:OwnershipTransferred:${BRIDGE}`,
      `BRIDGE:Paused:${BRIDGE}`,
      `BRIDGE:Unpaused:${BRIDGE}`,
    ],
  );
  assert.throws(
...
test('registers the new UKI marketplace separately from legacy BNB orders', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      `MARKETPLACE:Paused:${MARKETPLACE}`,
      `MARKETPLACE:Unpaused:${MARKETPLACE}`,
      `UKI_MARKETPLACE:CollectionAllowedUpdated:${UKI_MARKETPLACE}`,
      `UKI_MARKETPLACE:PaymentTokenAllowedUpdated:${UKI_MARKETPLACE}`,
      `UKI_MARKETPLACE:NativePaymentAllowedUpdated:${UKI_MARKETPLACE}`,
      `UKI_MARKETPLACE:FeeConfigUpdated:${UKI_MARKETPLACE}`,
      ...
      `UKI_MARKETPLACE:NativeFeesClaimed:${UKI_MARKETPLACE}`,
      `UKI_MARKETPLACE:OwnershipTransferred:${UKI_MARKETPLACE}`,
      `UKI_MARKETPLACE:OwnershipTransferStarted:${UKI_MARKETPLACE}`,
      `UKI_MARKETPLACE:Paused:${UKI_MARKETPLACE}`,
      `UKI_MARKETPLACE:Unpaused:${UKI_MARKETPLACE}`,
    ],
  );
  assert.throws(
...
test('registers TOKEN and TOKEN_V2 simultaneously without address fallback or TRON leakage', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      `TOKEN_V2:Approval:${TOKEN_V2}`,
      `TOKEN_V2:ApprovalForAll:${TOKEN_V2}`,
      `TOKEN_V2:OwnershipTransferred:${TOKEN_V2}`,
    ],
  );
  assert.throws(
    () => getContractEventConfigs(['BSC'], {
