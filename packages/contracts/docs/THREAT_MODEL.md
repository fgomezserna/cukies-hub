...
- `RewardsDistributor`
- `LiquidityLocker`
- `CukiesBridgeEndpoint` (Stage/Nile rehearsal only)
...
| Bridged Cukies | Source/destination endpoint custody | Double circulation, arbitrary mint/release, replay or stranded NFT. |
...
| T22 | Bridge relayer compromise | An allowlisted relayer can attest fabricated metadata or transfer ids and mint/release an NFT. | Explicit removable relayer allowlist, pause control, single-use ids, canonical metadata hash and full completion events. The Stage worker is one-way, lease/idempotency fenced and fail-closed. | Testnet only. Production requires approved quorum/proof design, multisig ownership, key rotation, monitoring and independent audit. |
| T23 | Bridge replay or double circulation | A request is completed twice or both chain representations remain user-accessible. | `processedTransfers`, custody-before-event, `lockedTransferByToken`, release only from tracked custody and round-trip invariant tests. | Reconcile ownership and circulating supply across both real testnets, including retries and delayed confirmations. |
| T24 | NFT is stranded by malformed transfer or recovery | Direct transfers bypass accounting or an operator recovers a live bridge position. | Direct safe transfers revert; untracked unsafe transfers are recoverable only while paused; tracked positions cannot use recovery. | Rehearse failed destination submission, DLQ, pause and manual recovery runbook on testnet. |
| T25 | Bridge fee is stale or excessive | Historical `10 TRX` overcharges users or underfunds BSC gas. | Endpoint exposes mutable exact `bridgePrice`; fee is forwarded atomically to the configured treasury. | Compute from measured destination gas and live BNB/TRX rate with approved buffer, publish formula and monitor drift. |
| T26 | Wrong-chain UI signature | Stage prompts a wallet transaction on BSC/TRON mainnet. | Public runtime requires Stage + chain 97 + Nile and rejects known legacy addresses and the event-only fixture; disabled mode mounts no on-chain operations. | Keep local config tests and perform a final wallet/network smoke on both testnets before enabling. |
| T27 | Ambiguous relayer submission is sent twice | RPC timeout occurs after BSC accepted `completeBridge`, then a retry submits a second transaction. | Submitted tx hash is persisted before confirmation; pending receipts are never resent and timeout goes to DLQ. Every retry first reads `processedTransfers`. | Exercise timeout/restart/replacement scenarios with the real BSC Testnet RPC and alert every submitted-timeout DLQ. |
| T28 | Metadata drifts between Tron and BSC | Relayer or stale Mongo data changes rarity, generation, skills, energy or health. | Source endpoint reads canonical collection metadata and emits a non-zero hash; worker and destination recompute the exact ABI hash before mint. | Compare rendered/API metadata against both endpoints in the real E2E receipt set. |
