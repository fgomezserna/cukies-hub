...
function requireEnv(name) {
  ...
  if (!value) throw new Error(`${name} is required for mainnet operational deploy`);
  return value;
}
...
function assertNoAttachEnv() {
  for (const name of ['UKI_TOKEN_ADDRESS', 'UKI_VESTING_VAULT_ADDRESS', 'UKI_PRESALE_ADDRESS']) {
    if (optionalEnv(name)) {
      throw new Error(`${name} must be empty for a fresh mainnet operational deploy. Use the handover/preflight scripts for existing contracts.`);
    }
  }
}
...
async function main() {
  ...
  const saleStart = optionalEnv('SALE_START') ? requireUint('SALE_START') : MAINNET_SALE_START;
  ...
  if (saleStart !== MAINNET_SALE_START) {
    throw new Error(`SALE_START must be ${MAINNET_SALE_START} (2026-06-15 17:00 Europe/Madrid). Received ${saleStart}`);
  }
  ...
  console.log(JSON.stringify({
    script: 'deploy-mainnet-operational.cjs',
    ...
    nextPhase: [
      ...
      'Run safe handover script before public launch if Safe addresses are ready.',
