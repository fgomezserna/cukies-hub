...
export interface RewardAccountingRepository {
  ...
  listDailyAmbassadorSourceSnapshots?(startsAt: Date, endsAt: Date): Promise<Record<string, string | null>>;
...
export function createMongoRewardAccountingRepository(
  ...
  return {
    ...
    async listDailyAmbassadorSourceSnapshots(startsAt, endsAt) {
      const rows = await sources.find({
        status: "settled",
        outcome: "completed",
        resultValid: true,
      ...
      }>({ _id: 0, sessionId: 1, ambassadorSnapshot: 1 }).toArray();
      return Object.fromEntries(rows.map((row) => {
        const snapshot = row.ambassadorSnapshot;
        const eligible = resolveAmbassadorSnapshotWallet(snapshot) !== null;
        return [`game-session:${row.sessionId}`, eligible ? snapshot.walletNormalized : null];
      }));
    },
    async listCukieParticipants(generation, startsAt, endsAt, observedAt = new Date()) {
      ...
      const referrals = await referralsFor([...new Set(rows.map((row) => row.wallet))], observedAt);
      return rows.map((row) => {
...
export class RewardAccountingService {
  ...
  closeNextDaily(input: {
    ...
    return this.runTransaction(async (repository) => {
      ...
      const ambassadorBySource = repository.listDailyAmbassadorSourceSnapshots
        ? await repository.listDailyAmbassadorSourceSnapshots(startsAt, endsAt)
        : {};
