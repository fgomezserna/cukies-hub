// Secret key for game communication (should be in environment variables)
const GAME_COMMUNICATION_KEY = process.env.NEXT_PUBLIC_GAME_COMMUNICATION_KEY || 'hyppie-secure-game-key-2024';
...
export async function generateMessageSignature(message: Omit<SecureGameMessage, 'signature'>): Promise<string> {
  ...
  // Fallback implementation using simple SHA-256 + key
...
export async function verifyMessageSignature(message: SecureGameMessage): Promise<boolean> {
  try {
    ...
    // Use simple string comparison for browser compatibility
