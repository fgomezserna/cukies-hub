    ...
    <>
      {error ? (
        <div role="alert" className="mb-3 flex items-center justify-between gap-4 rounded-[7px] border border-red-300/30 bg-red-950/25 px-4 py-3 text-sm text-red-100">
          <span>{error}</span>
          <button type="button" onClick={reload} className="min-h-10 px-2 font-black text-[#35eee2]">Reintentar</button>
        </div>
      ) : null}

      <section className="mb-3 overflow-hidden rounded-[8px] border border-[#b68b3c]/55 bg-[#061110]/94">
        <div className="grid gap-5 px-5 py-5 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div className="border-l-2 border-[#35eee2] pl-4">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#35eee2]">Competición semanal automática</p>
          ...
          <div className="flex flex-wrap gap-2 lg:justify-end">
            <Link href="/games/treasure-hunt/rules" className="inline-flex min-h-10 items-center rounded-[6px] border border-white/20 px-3.5 text-xs font-black text-[#f2eee7] hover:border-[#35eee2]/55">Ver reglas</Link>
            <Link href="/games/treasure-hunt" className="inline-flex min-h-10 items-center gap-2 rounded-[6px] border border-[#35eee2]/55 bg-[#0d5d57] px-3.5 text-xs font-black text-white hover:bg-[#137069]">Jugar <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
          </div>
        </div>
        <dl className="grid border-t border-white/15 sm:grid-cols-3 sm:divide-x sm:divide-white/15">
          <div className="px-5 py-4">
            <dt className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-[#969994]"><CalendarClock className="h-4 w-4 text-[#35eee2]" /> Periodo actual</dt>
          ...
          <div className="px-5 py-4">
            <dt className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-[#969994]"><Trophy className="h-4 w-4 text-[#35eee2]" /> Participantes clasificados</dt>
            <dd className="mt-1.5 font-mono text-xl font-black text-[#35eee2]">{data?.totalRankedWallets ?? '—'}</dd>
          </div>
        </dl>
      </section>

      {data?.participation ? (
        <section id="mi-participacion" className="mb-3 scroll-mt-6 rounded-[8px] border border-white/15 bg-[#091513] px-5 py-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#35eee2]">Tu semana</p>
            ...
            <div className="flex gap-2 text-xs font-bold">
              <span className="rounded-full border border-[#35eee2]/30 bg-[#35eee2]/10 px-3 py-1.5 text-[#35eee2]">{data.participation.poolCreditRuns} con créditos del pool</span>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/15 px-4 py-3 sm:px-5">
          <div className="inline-flex rounded-[7px] border border-white/15 bg-black/20 p-1" role="group" aria-label="Filtrar ranking">
            {([['general', 'General'], ['mine', 'Mi posición']] as const).map(([value, label]) => (
              <button key={value} type="button" aria-pressed={filter === value} onClick={() => setFilter(value)} className={cn('min-h-9 rounded-[5px] px-3 text-xs font-black transition', filter === value ? 'bg-[#35eee2]/15 text-[#35eee2]' : 'text-[#969994] hover:text-[#f2eee7]')}>{label}</button>
            ))}
          </div>
          ...
          <>
            <div className="space-y-2 p-3 sm:hidden">
              {data.entries.map((entry) => (
                <article key={`${entry.rank}:${entry.alias}`} className={cn('rounded-[7px] border border-white/15 bg-black/15 p-4', entry.isMe && 'border-[#35eee2]/45 bg-[#35eee2]/5')}>
                  <div className="flex items-center gap-3"><span className="font-mono font-black text-[#ffc240]">#{entry.rank}</span><p className="min-w-0 flex-1 truncate font-bold text-[#f2eee7]">{entry.alias}{entry.isMe ? <span className="ml-2 text-xs text-[#35eee2]">Tú</span> : null}</p><strong className="font-mono text-lg text-[#35eee2]">{Number(entry.scoreRaw).toLocaleString('es-ES')}</strong></div>
            ...
            <div className="hidden overflow-x-auto sm:block">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/10">
                  {data.entries.map((entry) => (
                    <tr key={`${entry.rank}:${entry.alias}`} className={cn(entry.isMe && 'bg-[#35eee2]/5')}><td className="px-5 py-4 font-mono font-black text-[#ffc240]">#{entry.rank}</td><td className="px-5 py-4 font-bold text-[#f2eee7]">{entry.alias}{entry.isMe ? <span className="ml-2 text-xs text-[#35eee2]">Tú</span> : null}</td><td className="px-5 py-4 text-[#aaa8a2]">{positionMeaning(entry.rank)}</td><td className="px-5 py-4 text-[#aaa8a2]">{entry.cukieSource === 'own' ? 'Propio' : 'Del pool'}</td><td className="px-5 py-4 text-right font-mono font-black text-[#35eee2]">{Number(entry.scoreRaw).toLocaleString('es-ES')}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
  ...
  return (
    <div className="mx-auto min-h-full w-full max-w-[68rem] pb-8">
      ...
      <div className="mb-4 grid w-full grid-cols-2 rounded-[7px] border border-white/15 bg-black/20 p-1 sm:w-fit" role="group" aria-label="Tipo de competición">
        {([['active', 'Semana actual'], ['finished', 'Torneos especiales']] as const).map(([value, label]) => <button key={value} type="button" aria-pressed={period === value} onClick={() => setPeriod(value)} className={cn('min-h-10 rounded-[5px] px-4 text-sm font-black transition', period === value ? 'bg-[#35eee2]/15 text-[#35eee2]' : 'text-[#969994] hover:text-[#f2eee7]')}>{label}</button>)}
      </div>
      {period === 'active' ? <ActiveTreasureHuntRankingsView /> : <section className="overflow-hidden rounded-[8px] border border-[#b68b3c]/55 bg-[#061110]/94"><TreasureHuntHistoryView /></section>}
    </div>
  );
}
