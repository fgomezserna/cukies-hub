            ...
            {nextTier && (
              <div className="mt-4">
                <div className="h-3 overflow-hidden rounded-full bg-black/45 border border-white/5">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-[#2ee8d6] to-[#7c3cff] transition-all duration-500 ease-out"
            ...
            <div className="grid gap-3.5">
              {purchaseRewards.map((reward) => {
                ...
                return (
                  <article
                    key={reward.amount}
                    className={`uki-hover-card grid gap-4 rounded-[12px] border ${reward.border} ${
                      reward.tone
                    } bg-gradient-to-r p-4 sm:grid-cols-[11rem_1.5rem_1fr] sm:items-center relative overflow-hidden transition-all duration-300 ${
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <strong className="font-headline text-xl font-black uppercase leading-none text-[var(--uki-cream)] block">
                          {reward.amountStr}
                        </strong>
                        <span className={`inline-block text-[0.62rem] font-bold uppercase px-2 py-0.5 rounded-[4px] border mt-1.5 ${reward.badgeColor}`}>
                          Tier {reward.tier}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
                      ...
                      {totalPurchased !== null && (
                        <div className="shrink-0 mt-2 sm:mt-0">
                          {isUnlocked ? (
                            <span className="inline-flex items-center gap-1.5 text-xs text-[var(--uki-cyan)] font-bold bg-[#2ee8d6]/10 border border-[#2ee8d6]/30 px-3 py-1.5 rounded-[6px]">
                              <CheckCircle2 className="h-4 w-4" />
                              Conseguido
                            </span>
                          ) : isNext ? (
                            <span className="inline-flex items-center gap-1.5 text-xs text-[var(--uki-gold)] font-bold bg-[#f2c34b]/10 border border-[#f2c34b]/30 px-3 py-1.5 rounded-[6px]">
                              <Sparkles className="h-4 w-4 animate-pulse" />
                              Siguiente tramo
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 text-xs text-[var(--uki-muted)] font-bold bg-white/5 border border-white/10 px-3 py-1.5 rounded-[6px]">
            ...
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {rarityRewards.map((rarity) => {
                ...
                return (
                  <article
                    key={rarity.name}
                    className={`uki-hover-card rounded-[12px] border ${rarity.border} ${rarity.glow} bg-[#09091a]/86 p-5 flex flex-col justify-between transition-all duration-300 relative overflow-hidden`}
                  >
