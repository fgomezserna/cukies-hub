...
const STATUS_COPY: Record<CommissionStatus, { label: string; helper: string }> =
  {
    registered: {
      label: "Registrada",
      helper: "Incluida en el cierre económico.",
    },
    preparing: {
      label: "En preparación",
      helper: "Se está preparando para publicarla.",
    },
    scheduled: {
      label: "Programada",
      helper: "Tendrá una fecha de cobro próxima.",
    },
    claimable: {
      label: "Disponible",
      helper: "Ya puedes cobrarla desde Premios.",
    },
    claimed: {
      label: "Cobrada",
      helper: "El envío a tu wallet quedó confirmado.",
    },
...
function Metric({
  ...
  return (
    <div className="min-w-0 px-5 py-5 sm:px-6">
      <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[var(--uki-muted)]">
        {label}
      </p>
      <p className="mt-2 font-headline text-3xl font-black tracking-[-0.03em] text-[var(--uki-cream)]">
        {value}
      </p>
...
function LoadingProgram() {
  return (
    <div
      ...
      aria-label="Cargando programa de embajadores"
    >
...
export function AmbassadorProgram({
  ...
  const [registrationInvitationCode, setRegistrationInvitationCode] =
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || body.status !== "ok" || !body.dashboard)
      ...
      setFeedback(
        "No podemos actualizar el programa ahora. No se ha guardado ningún cambio."
      );
    }
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/economy/v1/ambassadors/invitations/${encodeURIComponent(code)}`,
      {
        cache: "no-store",
  ...
  async function acceptInvitation() {
    if (!invitation || !consent || accepting || dashboard?.ownAttribution)
      return;
    setAccepting(true);
    setFeedback(null);
    try {
  ...
  const isOwnInvitation = Boolean(
    invitation &&
      dashboard?.profile.invitationCode === invitation.invitationCode
  );
  const canAcceptInvitation = Boolean(
    invitation &&
      registrationInvitationCode === invitation.invitationCode
  );
  ...
  return (
    <div className="uki-landing mx-auto min-h-full w-full max-w-[1480px] [background:transparent] pb-10 text-[var(--uki-cream)]">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.42fr)] lg:items-end">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-pretty text-sm font-semibold leading-relaxed text-[var(--uki-text)] sm:text-base">
              Recibes el {commissionPercent.toLocaleString("es-ES")}% de los
              premios elegibles que generen tus invitados. Ellos mantienen el
              100% de lo que ganen.
            </p>
          </div>
          <div className="border-l-2 border-[var(--uki-lilac)] pl-4">
            ...
            <p className="mt-2 text-sm font-semibold leading-relaxed text-[var(--uki-muted)]">
              Un nivel, sin comisiones encadenadas y sin cambios retroactivos.
              Los referidos confirmados en preventa ya están incluidos
              automáticamente.
            </p>
          </div>
        </div>
          ...
          <Panel innerClassName="p-5 sm:p-7">
            <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.48fr)] lg:items-center">
              <div>
                ...
                <h2
                  id="invitation-title"
                  className="mt-2 font-headline text-2xl font-black sm:text-3xl"
                >
                  Confirma quién te invitó
                </h2>
                {invitationState === "loading" ? (
                  <p className="mt-4 flex items-center gap-2 text-sm font-semibold text-[var(--uki-muted)]">
                    <SpinnerGap className="h-4 w-4 animate-spin text-[var(--uki-lilac)]" />{" "}
                    Comprobando la invitación…
                  </p>
                ) : invitationState === "error" ? (
                  <p
                    role="alert"
                    className="mt-4 flex items-start gap-2 text-sm font-semibold text-amber-200"
                  >
                    <Warning
                      className="mt-0.5 h-4 w-4 shrink-0"
                      weight="fill"
                    />{" "}
                    Esta invitación no existe o ya no puede utilizarse.
                  </p>
                ) : invitation ? (
                  <div className="mt-4 flex items-center gap-3">
                    <span className="grid h-11 w-11 place-items-center rounded-full border border-[var(--uki-lilac)]/30 bg-[var(--uki-lilac)]/10">
                      <Crown
                        className="h-5 w-5 text-[var(--uki-lilac)]"
                        weight="fill"
                      />
                    </span>
                    <div>
                      <p className="font-mono text-base font-black text-[var(--uki-cream)]">
                        {invitation.ambassadorWalletMasked}
                      </p>
                      <p className="mt-0.5 text-xs font-semibold text-[var(--uki-muted)]">
                        Será tu embajador directo
                      </p>
                    </div>
                  </div>
                ) : null}
              ...
              !isOwnInvitation ? (
                <div className="rounded-[12px] border border-[var(--uki-lilac)]/25 bg-[var(--uki-lilac)]/[0.055] p-4">
                  <label className="flex cursor-pointer items-start gap-3">
                    ...
                    <span className="text-xs font-semibold leading-relaxed text-[var(--uki-text)]">
                      Entiendo que esta relación es permanente y no podré
                      sustituirla más adelante.
                    </span>
                  </label>
                  <button
                    type="button"
                ...
                <div className="flex items-start gap-3 rounded-[12px] border border-white/10 bg-white/[0.035] p-4">
                  <LockKey
                    className="mt-0.5 h-5 w-5 shrink-0 text-[var(--uki-lilac)]"
                    weight="fill"
                  />
                  <p className="text-sm font-semibold leading-relaxed text-[var(--uki-muted)]">
                    Tu wallet ya tiene un embajador confirmado.
                  </p>
                </div>
              ) : isOwnInvitation ? (
                ...
                <div className="flex items-start gap-3 rounded-[12px] border border-white/10 bg-white/[0.035] p-4">
                  ...
                  />
                  <p className="text-sm font-semibold leading-relaxed text-[var(--uki-muted)]">
                    Esta wallet ya estaba registrada. Las invitaciones solo se
                    pueden confirmar durante el alta inicial.
                  </p>
                </div>
              ) : null}
            </div>
      ...
      {feedback ? (
        <div
          role="status"
          className="mt-5 flex items-start gap-3 rounded-[10px] border border-[var(--uki-lilac)]/25 bg-[var(--uki-lilac)]/[0.06] p-4 text-sm font-semibold text-[var(--uki-text)]"
        >
          <Check
            className="mt-0.5 h-4 w-4 shrink-0 text-[var(--uki-lilac)]"
            weight="bold"
          />{" "}
          {feedback}
        </div>
      ...
      (!walletAddress || walletType !== "evm") ? (
        <section className="grid min-h-[28rem] overflow-hidden rounded-[18px] border border-[var(--uki-lilac)]/25 bg-[#09060f] lg:grid-cols-[1.15fr_0.85fr]">
          <div className="flex flex-col justify-center p-6 sm:p-10">
            <UsersThree
              className="h-8 w-8 text-[var(--uki-lilac)]"
              weight="fill"
            />
            <h2 className="mt-5 max-w-xl font-headline text-3xl font-black sm:text-4xl">
              Conecta tu wallet para abrir tu programa
            </h2>
            <p className="mt-3 max-w-xl text-sm font-semibold leading-relaxed text-[var(--uki-muted)]">
              Verás tu enlace, los invitados confirmados y cada comisión desde
              que se registra hasta que llega a tu wallet.
            </p>
            <LandingWalletConnectButton
              evmOnly
              ambassadorInvitationCode={invitation?.invitationCode}
          ...
          <div className="relative grid min-h-[18rem] place-items-center border-t border-white/10 bg-[var(--uki-lilac)]/[0.045] lg:border-l lg:border-t-0">
            <Handshake
              className="h-28 w-28 text-[var(--uki-lilac)]/80"
              weight="duotone"
            />
          </div>
        </section>
      ...
      {requestState === "error" && walletAddress ? (
        <div
          role="alert"
          className="mt-7 flex flex-col gap-4 rounded-[14px] border border-amber-300/25 bg-amber-300/[0.06] p-5 sm:flex-row sm:items-center sm:justify-between"
        >
          <div>
            <p className="font-black">No podemos cargar tu programa ahora</p>
            <p className="mt-1 text-sm font-semibold text-[var(--uki-muted)]">
              Tus relaciones y comisiones no se han modificado.
            </p>
          </div>
          <button
            type="button"
            onClick={loadDashboard}
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[8px] border border-white/15 px-4 text-sm font-black"
          >
          ...
          <section aria-labelledby="summary-title" className="pt-7">
            <div className="flex items-end justify-between gap-4 pb-4">
              <div>
                ...
                <h2
                  ...
                  className="mt-2 font-headline text-2xl font-black sm:text-3xl"
                >
                  Lo que ya has generado
                </h2>
              </div>
              <button
                type="button"
                onClick={loadDashboard}
                className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.08em] text-[var(--uki-lilac)]"
              >
            ...
            <div className="grid overflow-hidden rounded-[16px] border border-[var(--uki-lilac)]/25 bg-[var(--uki-lilac)]/[0.055] sm:grid-cols-2 xl:grid-cols-4 sm:[&>*+*]:border-l sm:[&>*+*]:border-white/10">
              <Metric
                ...
                value={dashboard.referrals.length.toLocaleString("es-ES")}
                helper="Relaciones directas y permanentes"
              />
              <Metric
                label="En preparación"
                value={`${formatUki(
                  dashboard.commissions.totals.pendingRaw
                )} UKI`}
                helper="Registrado antes de su publicación"
              />
              <Metric
                label="Disponible"
                value={`${formatUki(
                  dashboard.commissions.totals.claimableRaw
                )} UKI`}
                helper="Ya puede cobrarse en Premios"
              />
              <Metric
                label="Cobrado"
                value={`${formatUki(
                  dashboard.commissions.totals.claimedRaw
                )} UKI`}
                helper="Confirmado en tu wallet"
              />
            </div>
          </section>
          ...
          <section className="grid gap-5 pt-7 lg:grid-cols-[1.08fr_0.92fr]">
            <Panel innerClassName="p-5 sm:p-7">
              <div className="flex items-start justify-between gap-4">
                ...
                <LinkSimple
                  className="h-7 w-7 shrink-0 text-[var(--uki-lilac)]"
                  weight="bold"
                />
              </div>
              <p className="mt-3 max-w-xl text-sm font-semibold leading-relaxed text-[var(--uki-muted)]">
                La persona invitada verá tu wallet abreviada y decidirá si
                confirma la relación. Tu dirección completa no aparece en el
                enlace.
              </p>
              <div className="mt-5 overflow-hidden rounded-[10px] border border-white/10 bg-black/25">
                <p className="break-all px-4 py-3 font-mono text-xs text-[var(--uki-text)]">
                  {invitationUrl}
                </p>
                <div className="grid border-t border-white/10 sm:grid-cols-2">
                  <button
                    ...
                    className="inline-flex min-h-11 items-center justify-center gap-2 px-4 text-sm font-black text-[var(--uki-lilac)] transition active:scale-[0.98]"
                  >
                    {copied ? (
                  ...
                  <button
                    ...
                    className="inline-flex min-h-11 items-center justify-center gap-2 border-t border-white/10 px-4 text-sm font-black text-[var(--uki-cream)] transition active:scale-[0.98] sm:border-l sm:border-t-0"
                  >
              ...
              {dashboard.ownAttribution ? (
                <div className="mt-5 rounded-[12px] border border-[var(--uki-lilac)]/25 bg-[var(--uki-lilac)]/[0.055] p-5">
                  <p className="font-mono text-lg font-black">
                    {dashboard.ownAttribution.ambassadorWalletMasked}
                  </p>
                  <p className="mt-2 text-sm font-semibold text-[var(--uki-lilac)]">
                    {sourceCopy(dashboard.ownAttribution.source)}
                  </p>
                  <p className="mt-2 text-xs font-semibold text-[var(--uki-muted)]">
                    Desde el {formatDate(dashboard.ownAttribution.acceptedAt)}.
                    Esta relación no puede sustituirse.
                  </p>
                </div>
              ) : (
                <div className="mt-5 flex items-start gap-3 rounded-[12px] border border-white/10 bg-white/[0.035] p-5">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm font-semibold leading-relaxed text-[var(--uki-muted)]">
                      Solo se asignará cuando abras un enlace válido y lo
                      confirmes con tu wallet.
                    </p>
                  </div>
                </div>
              )}
          ...
          <section aria-labelledby="referrals-title" className="pt-10">
            <div className="flex items-end justify-between gap-4 border-b border-white/10 pb-5">
              <div>
                ...
                <p className="mt-2 text-sm font-semibold text-[var(--uki-muted)]">
                  Aquí aparecen tanto las nuevas invitaciones como los referidos
                  confirmados durante la preventa.
                </p>
              </div>
              <span className="font-headline text-3xl font-black text-[var(--uki-lilac)]">
            ...
            {dashboard.referrals.length === 0 ? (
              <div className="flex min-h-44 flex-col items-center justify-center border-b border-white/10 px-5 py-8 text-center">
                <UsersThree
                  className="h-9 w-9 text-[var(--uki-lilac)]"
                  weight="duotone"
                />
                <p className="mt-4 font-headline text-xl font-black">
                  Todavía no tienes invitados confirmados
                </p>
                <p className="mt-2 max-w-lg text-sm font-semibold text-[var(--uki-muted)]">
                  Comparte tu enlace. La relación aparecerá aquí en cuanto la
                  otra wallet la confirme.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex min-w-0 items-center gap-3">
                      <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-[var(--uki-lilac)]/25 bg-[var(--uki-lilac)]/[0.07]">
                        <Check
                          className="h-4 w-4 text-[var(--uki-lilac)]"
                          weight="bold"
                        />
                      </span>
                      <div className="min-w-0">
                        <p className="font-mono text-sm font-black">
                          {referral.referredWalletMasked}
                        </p>
                        <p className="mt-1 text-xs font-semibold text-[var(--uki-muted)]">
                          {sourceCopy(referral.source)}
                        </p>
                      </div>
                    </div>
                    <p className="pl-[3.25rem] text-xs font-semibold text-[var(--uki-muted)] sm:pl-0">
                      Confirmado el {formatDate(referral.acceptedAt)}
                    </p>
                  </article>
                ))}
              </div>
          ...
          >
            <div className="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm font-semibold text-[var(--uki-muted)]">
                  Cada fila explica cuándo se registró y en qué estado se
                  encuentra.
                </p>
              </div>
              <Link
                href="/premios?category=ambassador"
                className="inline-flex min-h-10 items-center justify-center gap-2 rounded-[8px] border border-[var(--uki-lilac)]/35 px-4 text-sm font-black text-[var(--uki-lilac)]"
              >
            ...
            {dashboard.commissions.history.length === 0 ? (
              <div className="flex min-h-44 flex-col items-center justify-center border-b border-white/10 px-5 py-8 text-center">
                ...
                <p className="mt-2 max-w-lg text-sm font-semibold text-[var(--uki-muted)]">
                  Aparecerán cuando un invitado confirmado reciba un premio
                  elegible.
                </p>
              </div>
            ) : (
                ...
                {dashboard.commissions.history.map((entry) => {
                  ...
                  return (
                    <article
                      ...
                      className="grid gap-4 py-5 md:grid-cols-[minmax(0,1.2fr)_minmax(180px,0.7fr)_auto] md:items-center"
                    >
                      <div className="flex min-w-0 items-start gap-3">
                        ...
                        <div>
                          <p className="font-black">
                            {entry.kind === "weekly"
                              ? "Comisión semanal"
                              : "Comisión por premios"}
                          </p>
                          <p className="mt-1 text-xs font-semibold text-[var(--uki-muted)]">
                            {periodLabel(entry.periodId)} · {entry.sourceCount}{" "}
                            {entry.sourceCount === 1 ? "origen" : "orígenes"}
                          </p>
                        </div>
                      </div>
                      <div>
                        <p className="text-xs font-black uppercase tracking-[0.1em] text-[var(--uki-lilac)]">
                          {status.label}
                        </p>
                        <p className="mt-1 text-xs font-semibold text-[var(--uki-muted)]">
                          {status.helper}
                      ...
                      <p className="font-headline text-2xl font-black text-[var(--uki-cream)]">
                        {formatUki(entry.amountRaw)} UKI
                      </p>
                    </article>
                  );
                })}
              ...
              <div className="grid gap-px bg-white/10 lg:grid-cols-[0.8fr_1.2fr]">
                <div className="bg-[#0d0914] p-5 sm:p-7">
                  ...
                  <p className="mt-3 text-sm font-semibold leading-relaxed text-[var(--uki-muted)]">
                    El sistema conserva la relación y calcula la comisión usando
                    la atribución vigente cuando se crea la partida o se cierra
                    el periodo.
                  </p>
                </div>
                <ol className="divide-y divide-white/10 bg-[#0d0914]">
                  {[
                    [
                      ...
                      "El invitado acepta la relación una vez. Si venía de la preventa, ya aparece confirmada automáticamente.",
                    ],
                    [
                      "02",
                      "El invitado no pierde nada",
                      `El invitado recibe su premio completo y tú recibes un ${commissionPercent.toLocaleString(
                        "es-ES"
                      )}% adicional.`,
                    ],
                    [
                      ...
                      "La vinculación nueva solo afecta a premios posteriores. No hay segundo nivel ni comisión sobre otra comisión.",
                    ],
                    [
                      "04",
                      "Cobro desde Premios",
                      "La comisión se registra con el cierre correspondiente y se cobra mediante el mismo sistema de premios UKI.",
                    ],
                  ].map(([number, title, description]) => (
                    <li
                      key={number}
                      className="grid gap-2 p-5 sm:grid-cols-[44px_minmax(0,1fr)] sm:p-6"
                    >
