...
import { assertAmbassadorRuntime, validAmbassadorWallet } from "@/lib/uki-economy/ambassadors/rules";
...
export async function GET() {
  try {
    ...
    if (!session || session.walletType !== "evm" || typeof session.signedWalletAddress !== "string") {
    ...
    if (error instanceof UkiEconomyError) {
      return json(
        ...
        error.code === "CONFLICT" ? 409 : error.code === "VALIDATION" ? 400 : 503,
      );
    }
