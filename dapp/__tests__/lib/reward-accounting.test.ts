...
describe("reward accounting invariants", () => {
  ...
  it("calcula la comisión del jugador por source y permite sponsors distintos el mismo día", () => {
    ...
    const result = calculateDailyRewardSettlement({
      ...
      cukieSecondPlusParticipants: [],
      ambassadorBySource: {
        "game-session:a": { walletNormalized: wallet(1), ambassadorWalletNormalized: sponsorA },
        "game-session:b": { walletNormalized: wallet(1), ambassadorWalletNormalized: null },
      },
      sealedAt: new Date("2026-08-19T16:00:00.000Z"),
    });
...
describe("weekly prize", () => {
  ...
  it("permite a la raíz institucional cobrar sin etiquetarla como Master", () => {
    ...
    const result = calculateWeeklyPrize({
      periodId: "2026-W34", ruleVersion: CURRENT_REWARD_RULE.version, ruleConfigHash: CURRENT_REWARD_RULE.configHash, potRaw: "10000", ambassadorReserveRaw: "500",
      sourceDailyAccountingIds: DAILY_ACCOUNTING_IDS, results: [source],
