...
const config = loadRewardAccountingSchedulerConfig(process.env, hostname());
...
const heartbeats = client
  ? client.db(config.dbName).collection('reward_accounting_runtime_state')
  : null;
...
async function tick() {
  ...
  const canonical = [
    'cukies-economy-hmac-v1', config.keyId, 'POST', path, timestamp, nonce,
  ...
  if (!response.ok) throw new Error(`REWARD_ACCOUNTING_TICK_HTTP_${response.status}`);
}
  ...
  if (config.enabled) {
    try {
      ...
      const code = error instanceof Error
        ...
        : 'REWARD_ACCOUNTING_TICK_FAILED';
      try { await heartbeat('error', code); } catch {
        process.stderr.write(`${new Date().toISOString()} REWARD_ACCOUNTING_HEARTBEAT_FAILED\n`);
      }
