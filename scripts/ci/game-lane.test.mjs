...
function previous() {
  ...
  manifest.workersComposeHash = chooseDelivery({
    manifest: {...manifest, components: {...manifest.components, 'treasure-hunt': entry('treasure-hunt')}},
    previous: null,
    compose,
  }).workersComposeHash;
  return manifest;
...
test('game rollback preserves the prior image and rejects a wrong target before mutation', async () => {
  const target = {
    ...
    resourceUuid: 'game-resource',
    applicationId: '31',
    gitBranch: 'staging',
    repository: 'fgomezserna/cukies-hub',
    healthUrl: 'https://game.test/treasurehunt-game/api/health',
    readyUrl: 'https://game.test/treasurehunt-game/api/ready',
  ...
  let app = {
    ...
    health_check_path: '/treasurehunt-game/api/ready',
    docker_registry_image_name: 'registry.test/cukies-hub/treasure-hunt',
    docker_registry_image_tag: `sha256-${DIGEST_A.slice(7)}`,
  };
  let patches = 0;
  const client = {
    getApplication: async () => ({...app}),
    patchApplication: async (_uuid, body) => { patches += 1; app = {...app, ...body}; },
    patchEnvs: async () => {},
    start: async () => ({deployment_uuid: 'game-2'}),
...
test('state upgrade keeps the aggregate commit and records independent web/game commits', () => {
  const prior = {
    ...
    commit: SHA_A,
    configHash: HASH,
    components: Object.fromEntries(CI_COMPONENTS.filter((component) => component !== 'treasure-hunt').map((component) => [component, entry(component)])),
  };
