...
test('Dockerfile solo en dapp descarta Nx global y conserva el catálogo restante', () => {
  const baseDockerfile = [
    'FROM node:22-bookworm-slim AS base',
    '',
    'FROM base AS dapp',
  ...
  assert.deepEqual(plan.reuse.map((entry) => entry.component), COMPONENTS.filter((component) => component !== 'dapp'));
...
test('release plan selecciona dapp por config nueva y reutiliza los demás digests', () => {
  ...
  assert.deepEqual(plan.reuse.map((entry) => entry.component), COMPONENTS.filter((component) => !['dapp', 'treasure-hunt'].includes(component)));
