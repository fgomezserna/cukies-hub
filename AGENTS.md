...
- `infrastructure/ci/components.json` declares this branch's images: dapp, chain-indexer, cuki-card-worker and treasure-hunt. Existing schedulers use the DApp image and their disabled profiles stay unchanged. Staging has additional packages; do not import them solely for this infrastructure change.
...
- Treasure Hunt uses its own image and resource: app13 in production, app31 in staging. Its changes preserve web/workers; keep game basePath and runtime guards aligned with the target. The initial Nixpacks conversion requires a private snapshot and `git_commit_sha=HEAD`, without stopping the serving container.
- Git autodeploy on app12 and app13 must be OFF before the first infrastructure merge into main, even while the CI delivery gate is false. Preserve their running containers while CI builds the candidates. `CUKIES_DELIVERY_MODE=rolling` and `CUKIES_IMAGE_DEPLOY_ENABLED=true` enable the new lane; false builds without delivery. Keep production and staging GitHub Environments, config hashes and state paths separate.
